...
metrics = ["window_ms", "inbox-listed_at_ms", "switch-listed_ms", "search-listed_ms", "open-rendered_ms", "open-painted_ms", "rss_peak_mib", "pss_end_mib", "cpu_total_s", "idle_cpu_pct"]
...
for metric in metrics:
    ...
    for c, m in clients:
        ...
        cells.append((f"{statistics.median(vals):.1f} (best {min(vals):.1f}, n={len(vals)})" if metric in ("cpu_total_s", "idle_cpu_pct") else f"{statistics.median(vals):.0f} (best {min(vals):.0f}, n={len(vals)})") if vals else "-")
