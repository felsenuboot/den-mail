...
With `FASTMAIL_GTK_SESSION_URL` set the app registers a non-unique GApplication,
...
from a script (see `fastmail_gtk/autopilot.py` for the commands: `select`,
`mailbox`, `search`, `action`, `compose`, `from-popup`, `theme`, `context-menu`,
`thread-menu`, `resize`, `quit`, …). The screenshots in `data/screenshots/` are
taken that way inside a headless `cage` compositor:

```
WLR_BACKENDS=headless WLR_RENDERER=pixman WLR_LIBINPUT_NO_DEVICES=1 \
  cage -- sh -c './bin/fastmail-gtk & sleep 6; grim shot.png; kill %1'
```
...
| `FASTMAIL_GTK_DEBUG=1` | log every JMAP request (method names and sizes, never bodies) |
| `FASTMAIL_GTK_NO_WEBKIT=1` | force the text renderer for HTML mail |
| `FASTMAIL_GTK_AUTOPILOT` | script to run after start-up (see above) |
